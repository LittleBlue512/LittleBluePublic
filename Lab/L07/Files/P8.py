...
def calKineticPower(A, rho, Q):
    return (rho*Q**3) / (2*A**2)


def mighty_river(rivers):

    P_kinetic = {}
    for riverName, riverInfo in rivers.items():
        P_kinetic[riverName] = calKineticPower(riverInfo[0], riverInfo[1], riverInfo[2])
